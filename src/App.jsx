...
export default function App() {
  ...
  const apostasPorCategoria = apostas.reduce((acc, aposta) => {
    ...
    if (aposta.vencedor_real) acc[aposta.categoria].vencedor_real = aposta.vencedor_real;
    return acc;
  }, {});
  ...
  const placar = apostas.reduce((acc, aposta) => {
    ...
    if (aposta.vencedor_real) {
      ...
      const acertouGanha = !apostouPerde && aposta.indicado === aposta.vencedor_real;
      const acertouPerde = apostouPerde && aposta.indicado !== aposta.vencedor_real;
  ...
  const deletarAposta = async (id) => {
    ...
    const { error } = await supabase.from('apostas_oscar').delete().eq('id', id);
    if (!error) carregarApostas();
  };

  if (showForm) {
    return <NovaAposta onClose={() => setShowForm(false)} onApostaSalva={carregarApostas} />;
  }
    ...
    <div className="bg-[#121212] text-white min-h-[100dvh] pb-28 font-manrope">
        ...
        <div className="max-w-md mx-auto flex flex-col">
          ...
          <div className="flex items-center gap-4 overflow-x-auto pb-2 scrollbar-hide">
            {apostadores.length === 0 ? (
              <span className="text-sm text-zinc-500 font-medium">Aguardando apostas...</span>
            ) : (
              apostadores.map((pessoa, index) => (
                <div key={pessoa.nomeExibicao} className="flex items-center gap-4 shrink-0">
                  <div className="flex flex-col">
                    <span className="text-xs text-zinc-400 font-bold uppercase tracking-wide truncate max-w-[80px]">{pessoa.nomeExibicao}</span>
                    <span className="text-[#f2cc0d] font-extrabold text-lg">R$ {pessoa.ganhos.toFixed(2)}</span>
                  </div>
                  {index < apostadores.length - 1 && <div className="h-8 w-[1px] bg-white/5"></div>}
                </div>
              ))
            )}
          </div>
      ...
      <main className="max-w-md mx-auto px-4 py-6 space-y-6">
        {Object.entries(apostasPorCategoria).map(([categoria, dados]) => {
          ...
          return (
            <article key={categoria} className={`bg-[#1e1e1e] rounded-2xl overflow-hidden shadow-2xl border transition-colors ${isFinalizado ? 'border-[#f2cc0d]/30' : 'border-white/5'}`}>

              {/* CARD HEADER */}
              <header className="p-4 border-b border-white/5 flex flex-col gap-1">
                  <h2 className="text-xl font-extrabold tracking-tight text-white leading-tight">{categoria}</h2>
                  <div className="flex items-center gap-3 mt-1">
                    <span className={`px-2 py-0.5 text-[9px] font-bold uppercase rounded tracking-widest border ${isFinalizado ? 'bg-[#f2cc0d]/20 text-[#f2cc0d] border-[#f2cc0d]/30' : 'bg-zinc-800 text-zinc-400 border-zinc-700'}`}>
                      {isFinalizado ? 'Finalizado' : 'Pendente'}
                    </span>
                    <div className="flex items-center gap-1">
                      <span className="text-[#f2cc0d] text-[11px] font-semibold uppercase tracking-wider">Valor em Jogo:</span>
                      <span className="text-[#f2cc0d] text-[11px] font-extrabold">R$ {dados.valor.toFixed(2)}</span>
                    </div>
                  </div>
              </header>
              ...
              <section className="p-4 space-y-3">
                {dados.palpites.map((palpite) => {
                  ...
                  if (isFinalizado) {
                    const acertouGanha = !apostouPerde && palpite.indicado === dados.vencedor_real;
                    const acertouPerde = apostouPerde && palpite.indicado !== dados.vencedor_real;
                    ...
                    if (acertouGanha || acertouPerde) {
                      ...
                      icon = (
                        <svg className="text-green-500" fill="none" height="12" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" viewBox="0 0 24 24" width="12" xmlns="http://www.w3.org/2000/svg">
                          <polyline points="20 6 9 17 4 12"></polyline>
                        </svg>
                      );
                    } else {
                      containerClass = "bg-red-500/5 border-red-500/20 opacity-70";
                  ...
                  return (
                    <div key={palpite.id} className={`p-4 rounded-xl border flex flex-col gap-1 transition-all relative ${containerClass}`}>
                      {/* Canto superior direito: Tag + Lixeira */}
                      <div className="absolute top-4 right-4 flex items-center gap-2">
                        <span className={`px-2 py-0.5 text-[8px] font-black uppercase rounded-full border tracking-tighter shrink-0 ${apostouPerde ? 'bg-orange-500/10 text-orange-400 border-orange-500/20' : 'bg-blue-500/10 text-blue-400 border-blue-500/20'}`}>
                          Apostou que {apostouPerde ? 'perde' : 'ganha'}
                        </span>
                        {!isFinalizado && (
                          <button onClick={() => deletarAposta(palpite.id)} className="text-zinc-600 hover:text-red-400 transition-colors p-1 bg-white/5 rounded-full" title="Apagar Palpite">
                      ...
                      {/* Nome e Ícone */}
                      <div className="flex items-center gap-1.5 min-w-0 pr-[110px]"> {/* pr- pág o espaço da tag+lixeira */}
                        <span className={`text-xs font-bold uppercase tracking-wide truncate ${nameClass}`}>{palpite.apostador}</span>
                        {icon}
                      </div>

                      {/* Filme */}
                      <p className={`text-base font-bold ${movieClass} pr-[110px]`}>{palpite.indicado}</p>
                    </div>
                  );
                })}
              </div>
              ...
              <footer className="bg-black/20 p-4 border-t border-white/5">
                {!isFinalizado ? (
                  <>
                    <label className="block text-[9px] font-black text-zinc-500 uppercase tracking-[0.2em] mb-3">Mesário: Vencedor</label>
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        onClick={() => definirVencedor(categoria, document.getElementById(`winner-${categoria}`).value)}
                  ...
                  <div className="flex justify-between items-center gap-3">
                    <div className="flex flex-col min-w-0">
                      <span className="block text-[9px] font-black text-[#f2cc0d] uppercase tracking-[0.2em] mb-1">Vencedor Oficial</span>
                      <span className="text-base font-bold text-white truncate">{dados.vencedor_real}</span>
                    </div>
                    ...
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                    </button>
                  </div>
                )}
              </footer>
